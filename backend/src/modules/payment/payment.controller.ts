...
export const sslcommerzSuccessController: RequestHandler = catchAsync(async (req, res) => {
    ...
    try {
        ...
        if (result.success) {
            const redirectUrl = new URL(`${frontendUrl}/payment-status/success`);
            ...
            return res.redirect(
                `${frontendUrl}/payment-status/failed?status=failed&message=${encodeURIComponent(result.message)}`
            );
        }
    } catch (error) {
        console.error("[v0] SSLCommerz Success Error:", error);
        return res.redirect(
            `${frontendUrl}/payment-status/failed?status=error&message=Payment%20processing%20error`
        );
    }
});
...
export const sslcommerzCancelController: RequestHandler = catchAsync(async (req, res) => {
    ...
    return res.redirect(
        `${frontendUrl}payment-status/failed?status=cancelled&message=Payment%20cancelled&tran_id=${tran_id}`,
    )
})
...
export const sslcommerzFailController: RequestHandler = catchAsync(async (req, res) => {
    ...
    const redirectUrl = new URL(`${frontendUrl}/payment-status/failed?status=failed&message=Payment%20failed&tran_id=${tran_id}`);
    redirectUrl.searchParams.append("amount", result.payment.amount.toString());
    return res.redirect(redirectUrl.toString());
}
)
